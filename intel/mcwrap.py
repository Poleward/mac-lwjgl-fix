...
def copy_native_libs(dest_dir):
    pathExists = os.path.exists(dest_dir)
    if pathExists:
        shutil.rmtree(dest_dir)
    pathlib.Path(dest_dir).mkdir(parents=True, exist_ok=True)
    logging.info('copying native libs from {} to {}'.format(native_libs_dir(), dest_dir))
    for f in glob.glob('{}/*.dylib'.format(native_libs_dir())):
